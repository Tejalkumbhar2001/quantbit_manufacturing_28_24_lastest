...
class ComponentWorkOrder(Document):
	...
	# Fetch Component Manifest Finished Item to Component Work Order Doctype After Adding finished item details
	@frappe.whitelist()
	...
	def quantity_in_cri_from_fid(self):
		for i in self.get("finished_item_details"):
			for row in self.get("component_raw_item" , filters = {"reference_id":i.reference_id}):
				row.quantity = (i.updatedqty * row.actual_quantity)/i.qty
	...
	def Manufacturing_stock_entry(self):
		for f in self.get("finished_item_details"):
			...
			for i in self.get("component_raw_item" , filters = {"reference_id":f.reference_id}):	
				doc.append("items", {
					"s_warehouse": i.source_warehouse,
					"item_code": i.item_code,
					"item_name": i.item_name,
					"qty": f.okqty * i.actual_quantity ,
				})
			doc.append("items", {
